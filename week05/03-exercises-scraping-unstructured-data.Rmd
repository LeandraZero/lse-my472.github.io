...
The first example will be to collect only a single element of the HTML code, i.e. here the caption of the first image. It is on the right hand side of the website and begins with "Noether grew up in ...". Inspect the caption with your browser, copy its CSS selector, and scrape the caption by completing the code below:

```{r}
first_caption_element <- html_noether %>% html_elements(css = "div.mw-parser-output > figure:nth-child(11)")
...
```{r}
all_caption_elements <- html_noether %>% html_elements(css = "figcaption")
...
```{r}
all_lhs_caption_elements <- html_noether %>% html_elements(css = ".mw-halign-left figcaption")
...
data_elements_feynman <- html_feynman %>% html_elements(css = ________)
```

Next, extract the text from these elements:

```{r}
labels_feynman <- label_elements_feynman %>% ________
data_feynman <- data_elements_feynman %>% ________
```
